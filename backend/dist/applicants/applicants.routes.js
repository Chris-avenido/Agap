...
router.get('/:id', async (req, res, next) => {
    ...
    if (isNaN(id))
        return next();
    try {
        const applicant = await applicants_service_1.ApplicantsService.findOne(id);
        if (!applicant)
...
router.get('/:id/applications', async (req, res, next) => {
    ...
    if (isNaN(id))
        return next();
    try {
        const applications = await applicants_service_1.ApplicantsService.findApplications(id);
...
router.post('/:id/save-job', async (req, res, next) => {
    ...
    if (isNaN(id))
        return next();
    try {
        const result = await applicants_service_1.ApplicantsService.toggleSavedJob(id, req.body.positionId);
        res.json({ success: true, data: result });
    }
    catch (error) {
...
router.get('/:id/saved-jobs', async (req, res, next) => {
    ...
    if (isNaN(id))
        return next();
    try {
        const savedJobs = await applicants_service_1.ApplicantsService.findSavedJobs(id);
...
router.post('/:id/change-password', async (req, res, next) => {
    ...
    try {
        ...
        await applicants_service_1.ApplicantsService.changePassword(id, currentPassword, newPassword);
...
router.put('/:id', async (req, res, next) => {
    ...
    if (isNaN(id))
        return next();
    try {
        const applicant = await applicants_service_1.ApplicantsService.update(id, req.body);
