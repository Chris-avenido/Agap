...
class ApplicantsServiceClass {
    ...
    async create(data) {
        ...
        const oi = data.other_information || {};
        const fb = data.family_background || {};
        const fbSpouse = fb.spouse || {};
        const fbFather = fb.father || {};
        const fbMother = fb.mother || {};
        const result = await database_1.pool.query(`
      INSERT INTO applicants (
        ...
        spouse_surname, spouse_first_name, spouse_middle_name, spouse_name_extension, spouse_occupation, spouse_employer_business, spouse_business_address, spouse_telephone,
        father_surname, father_first_name, father_middle_name, father_name_extension,
        mother_first_name, mother_middle_name, mother_maiden_surname,
        name_extension, height, weight, agency_employee_no, citizenship_type
      ) VALUES (
    ...
    `, [
            ...
            data.questionnaire_responses ? JSON.stringify(data.questionnaire_responses) : null,
            data.family_background ? JSON.stringify(data.family_background) : null,
            fbSpouse.surname || data.spouse_surname || null,
            fbSpouse.first_name || data.spouse_first_name || null,
            fbSpouse.middle_name || data.spouse_middle_name || null,
            fbSpouse.name_extension || data.spouse_name_extension || null,
            fbSpouse.occupation || data.spouse_occupation || null,
            fbSpouse.employer_business_name || fbSpouse.employer || data.spouse_employer_business || null,
            fbSpouse.business_address || data.spouse_business_address || null,
            fbSpouse.telephone_no || fbSpouse.telephone || data.spouse_telephone || null,
            fbFather.surname || data.father_surname || null,
            fbFather.first_name || data.father_first_name || null,
            fbFather.middle_name || data.father_middle_name || null,
            fbFather.name_extension || data.father_name_extension || null,
            fbMother.first_name || data.mother_first_name || null,
            fbMother.middle_name || data.mother_middle_name || null,
            fbMother.maiden_surname || data.mother_maiden_surname || null,
            oi.extension_name || oi.extensionName || data.name_extension || data.extension_name || null,
            oi.height || data.height || null,
            oi.weight || data.weight || null,
            oi.agency_employee_no || oi.agencyEmployeeNo || data.agency_employee_no || null,
            oi.citizenship_type || oi.citizenshipType || data.citizenship_type || null
        ]);
        const applicant = result.rows[0];
        if (data.positionId) {
            const appId = require('crypto').randomUUID();
    ...
    async update(id, data) {
        ...
        if (data.other_information) {
            const oi = data.other_information;
            addField('name_extension', oi.extension_name || oi.extensionName || null);
            addField('height', oi.height || null);
            addField('weight', oi.weight || null);
            addField('agency_employee_no', oi.agency_employee_no || oi.agencyEmployeeNo || null);
            addField('citizenship_type', oi.citizenship_type || oi.citizenshipType || null);
        }
        if (data.family_background) {
            const fb = data.family_background;
            if (fb.spouse) {
                addField('spouse_surname', fb.spouse.surname || null);
                addField('spouse_first_name', fb.spouse.first_name || null);
                addField('spouse_middle_name', fb.spouse.middle_name || null);
                addField('spouse_name_extension', fb.spouse.name_extension || null);
                addField('spouse_occupation', fb.spouse.occupation || null);
                addField('spouse_employer_business', fb.spouse.employer_business_name || fb.spouse.employer || null);
                addField('spouse_business_address', fb.spouse.business_address || null);
                addField('spouse_telephone', fb.spouse.telephone_no || fb.spouse.telephone || null);
            }
            if (fb.father) {
                addField('father_surname', fb.father.surname || null);
                addField('father_first_name', fb.father.first_name || null);
                addField('father_middle_name', fb.father.middle_name || null);
                addField('father_name_extension', fb.father.name_extension || null);
            }
            if (fb.mother) {
                addField('mother_first_name', fb.mother.first_name || null);
                addField('mother_middle_name', fb.mother.middle_name || null);
                addField('mother_maiden_surname', fb.mother.maiden_surname || null);
            }
        }
        if (fields.length === 0)
            return null;
