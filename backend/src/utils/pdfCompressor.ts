 ...
 */
export const compressPdf = async (inputBuffer: Buffer, dpi: number = 150): Promise<Buffer> => {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const pythonProcess = spawn(pythonExecutable, [scriptPath, inputPath, outputPath, dpi.toString()]);
    ...
    if (outputBuffer.length < inputBuffer.length) {
      console.log(`[PDF Compression] Shrunk from ${inputBuffer.length} to ${outputBuffer.length} bytes`);
      return outputBuffer;
    } else {
      console.log(`[PDF Compression] Compression did not reduce size. Keeping original.`);
      return inputBuffer;
    }
    ...
    try {
      if (await fs.stat(inputPath).then(() => true).catch(() => false)) {
